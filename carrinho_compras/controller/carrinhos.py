...
async def adiciona_itens_carrinho(carrinho: CarrinhoRequest) -> CarrinhoCompleto:
    ...
    if carrinho.produto:
        dados_produto = await produto.pega_codigo_cor_tamanho(
            carrinho.produto.codigo, carrinho.produto.cor, carrinho.produto.tamanho
        )
    ...
    valor_total = round(
        carrinho.produto.quantidade * float(dados_produto.preco) + carrinho.valor_frete,
        2,
    )
...
async def busca_produto_carrinho(
    produtos_carrinho: list[CarrinhoItemSchema], produto_procurado: CarrinhoItemSchema
):

    for produto in produtos_carrinho:
        if (
            produto.codigo == produto_procurado.codigo
            and produto.cor == produto_procurado.cor
            and produto.tamanho == produto_procurado.tamanho
        ):
...
async def exclui_item_carrinho(
    produto_excluir: ExclusaoProdutoRequest, email_cliente: EmailStr
):
    ...
    dados_produto_carrinho = await busca_produto_carrinho(
        dados_carrinho.produtos, produto_excluir
    )
    ...
    if len(dados_carrinho.produtos) == 1:
        ...
        valor_total = round(
            dados_carrinho.valor_total
            - dados_produto_carrinho.preco_unitario * dados_produto_carrinho.quantidade,
            2,
        )

        quantidade_total = (
            dados_carrinho.quantidade_total - dados_produto_carrinho.quantidade
        )
