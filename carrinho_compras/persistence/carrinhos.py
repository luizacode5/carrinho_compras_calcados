...
async def exclui_carrinho(email_cliente: EmailStr):
    try:
        resultado = await COLECAO_CARRINHOS.delete_one({"email_cliente": email_cliente})
...
async def busca_carrinho_cliente(email_cliente: EmailStr) -> CarrinhoCompleto:
    try:
        resultado = await COLECAO_CARRINHOS.find_one({"email_cliente": email_cliente})
...
async def busca_carrinhos_por_produto(
    ...
    try:
        ...
        cursor_pesquisa = (
            COLECAO_CARRINHOS.find({"produtos": {"$elemMatch": filtro_produto}})
...
async def busca_produtos_populares(
    ...
    try:
        ...
        cursor_pesquisa = COLECAO_CARRINHOS.aggregate(
            [
                {"$unwind": "$produtos"},
                {
                    "$group": {
