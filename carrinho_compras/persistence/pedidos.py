...
async def busca_pedido_por_id(id_pedido: str) -> PedidoSchema:
    try:
        resultado = await COLECAO_PEDIDOS.find_one({"_id": ObjectId(id_pedido)})
        ...
        if resultado:
            ...
            return pedido
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="O pedido não foi encontrado"
        )
...
async def busca_pedidos_por_cliente(
    email_cliente: EmailStr, registros_pular: int, qtde_por_pagina: int
) -> ListaPedidos:
    try:
        lista_pedidos = ListaPedidos()

        cursor_pesquisa = (
            COLECAO_PEDIDOS.find({"email_cliente": email_cliente})
...
async def busca_pedidos_por_produto(
    filtro_produto: dict, registros_pular: int, qtde_por_pagina: int
) -> ListaPedidos:
    try:
        lista_pedidos = ListaPedidos()

        cursor_pesquisa = (
            COLECAO_PEDIDOS.find({"produtos": {"$elemMatch": filtro_produto}})
            .skip(registros_pular)
            .limit(qtde_por_pagina)
            .sort("data_criacao_pedido", DESCENDING)
        )
...
async def busca_produtos_mais_vendidos(
    registros_pular: int, qtde_por_pagina: int
) -> ProdutosMaisVendidos:
    try:
        lista_produtos = ProdutosMaisVendidos()
        lista_produtos.produtos = []

        cursor_pesquisa = COLECAO_PEDIDOS.aggregate(
            [
                {"$unwind": "$produtos"},
                {
                    "$group": {
...
async def busca_total_pedidos_por_cliente(
    registros_pular: int, qtde_por_pagina: int, ordenacao_filtro: str
) -> TotalPedidoClientes:
    try:
