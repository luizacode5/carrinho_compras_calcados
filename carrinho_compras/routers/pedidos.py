...
rota_pedidos = APIRouter(prefix="/pedido", tags=["Pedido"])
...
async def busca_pedido_por_id(id_pedido: str = Query(description="Id do pedido")):
...
async def busca_pedidos_por_cliente(
    ...
    qtde_por_pagina: int = Query(
        default=50, description="Quantidade de registros por página. Valor máximo: 50"
    ),
):
    resultado = await pedidos.busca_pedidos_por_cliente(
        email_cliente, numero_pagina, qtde_por_pagina
    )
    return resultado
...
async def busca_pedidos_por_produto(
    ...
    tamanho_produto: str = Query(
        default=None, description="Tamanho/numeração do produto"
    ),
    numero_pagina: int = Query(default=0, description="Número da página"),
    qtde_por_pagina: int = Query(
        default=50, description="Quantidade de registros por página. Valor máximo: 50"
...
async def busca_total_pedidos_por_cliente(
    ordenacao: str = Query(
        default="quantidade", description='Ordenação. Usar "quantidade" ou "valor"'
    ),
    numero_pagina: int = Query(default=0, description="Número da página"),
    qtde_por_pagina: int = Query(
        default=50, description="Quantidade de registros por página. Valor máximo: 50"
    ...
    resultado = await pedidos.busca_total_pedidos_por_cliente(
        numero_pagina, qtde_por_pagina, ordenacao
    )
    return resultado
